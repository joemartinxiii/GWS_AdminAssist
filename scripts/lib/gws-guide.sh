...
guide_oauth_setup() {
  ...
  echo ""
  echo "Step 1 — OAuth consent screen scopes (identity only — who the user is)."
  echo "         Workspace APIs use domain-wide delegation in Step 3, not these scopes."
  echo "         Copy/paste EXACTLY (comma-delimited, no spaces after commas):"
  echo ""
  ...
  echo "  - Authorized redirect URI: enter the placeholder below for now."
  echo "    After the deploy you'll be prompted to add the REAL Cloud Run URL"
  echo "    (the app's URL isn't known until it's deployed)."
...
guide_dwd_setup() {
  ...
  echo "   ${sa_client_id}"
  echo ""
  echo "4. OAuth scopes — copy/paste EXACTLY (comma-delimited, no spaces after commas)."
  echo "   These come from scripts/lib/scopes.sh (DWD_SCOPES) and must match the app."
  echo "   If deploy later prints an updated scope string, re-paste it here."
  echo ""
  echo "$DWD_SCOPES"
  echo ""
