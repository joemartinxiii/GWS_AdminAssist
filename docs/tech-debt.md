...
- **Security audit last-run + waivers are org-durable** — explicit **Run audit** (super admin) persists `security-audit/latest.json`; waivers live in `security-audit/waivers.json` and outlive re-runs. Export uses the cache. Policy API failures surface as a banner + manual fallbacks (no synthetic 403 checklist row). Checks carry severity + client-facing rationale. (`frontend/src/pages/SecurityAudit.tsx`, `backend/src/services/securityAuditStore.ts`)
